...
```sh
tap pack add inem/tap-pack-chatgpt-search@0.1.3
...
before enabling local executable code. The `@0.1.3` suffix is required while
...
curl -fL \
  https://github.com/inem/tap-pack-chatgpt-search/releases/download/v0.1.3/chatgpt.search-0.1.3.tap-pack \
  -o /tmp/chatgpt.search-0.1.3.tap-pack
```
...
PYTHONPATH="$CORE" python3 -B -m tap_core.pack_store build . \
  --output /tmp/chatgpt.search-0.1.3.tap-pack
```

Enable it and confirm declarative discovery:

```sh
"$CORE/tap" --profile "$PROFILE" pack install /tmp/chatgpt.search-0.1.3.tap-pack
